    ...
    context(`${contract}`, function () {
      ...
      describe("EIP-165 support", async function () {
        it("supports IERC721", async function () {
          expect(await this.erc721a.supportsInterface("0x80ac58cd")).to.eq(
            true
          );
        });

        it("supports ERC721Metadata", async function () {
          expect(await this.erc721a.supportsInterface("0x5b5e139f")).to.eq(
            true
          );
        });

        it("does not support ERC721Enumerable", async function () {
          expect(await this.erc721a.supportsInterface("0x780e9d63")).to.eq(
            false
          );
        });

        it("does not support random interface", async function () {
          expect(await this.erc721a.supportsInterface("0x00000042")).to.eq(
            false
          );
        });
      });
      ...
      context("with minted tokens", async function () {
        ...
        describe("ERC721Metadata support", async function () {
          ...
          describe("tokenURI", async function () {
            ...
            it("reverts when tokenid is invalid", async function () {
              await expect(
                this.erc721a["tokenURI(uint256)"](42)
              ).to.be.reverted;
            });
          });
        });
        ...
        describe("exists", async function () {
          ...
          it("verifies invalid tokens", async function () {
            expect(
              await this.erc721a.exists(6 + this.startTokenId)
            ).to.be.false;
          });
        });
        ...
        describe("balanceOf", async function () {
          it("returns the amount for a given address", async function () {
            expect(await this.erc721a.balanceOf(this.owner.address)).to.equal(
              "0"
            );
            expect(await this.erc721a.balanceOf(this.addr1.address)).to.equal(
              "1"
            );
            expect(await this.erc721a.balanceOf(this.addr2.address)).to.equal(
              "2"
            );
            expect(await this.erc721a.balanceOf(this.addr3.address)).to.equal(
              "3"
            );
          });
        ...
        describe("_numberMinted", async function () {
          it("returns the amount for a given address", async function () {
            expect(
              await this.erc721a.numberMinted(this.owner.address)
            ).to.equal("0");
            expect(
              await this.erc721a.numberMinted(this.addr1.address)
            ).to.equal("1");
            expect(
              await this.erc721a.numberMinted(this.addr2.address)
            ).to.equal("2");
            expect(
              await this.erc721a.numberMinted(this.addr3.address)
            ).to.equal("3");
          });
        });
        ...
        describe("aux", async function () {
          it("get and set works correctly", async function () {
            ...
            expect(await this.erc721a.getAux(this.owner.address)).to.equal(
              uint64Max
            );
        ...
        describe("ownerOf", async function () {
          it("returns the right owner", async function () {
            expect(await this.erc721a.ownerOf(0 + this.startTokenId)).to.equal(
              this.addr1.address
            );
            expect(await this.erc721a.ownerOf(1 + this.startTokenId)).to.equal(
              this.addr2.address
            );
            expect(await this.erc721a.ownerOf(5 + this.startTokenId)).to.equal(
              this.addr3.address
            );
          });
        ...
        describe("approve", async function () {
          ...
          it("rejects an unapproved caller", async function () {
            await expect(
              this.erc721a.approve(this.addr2.address, this.tokenId)
        ...
        context("test transfer functionality", function () {
          const testSuccessfulTransfer = function (transferFn) {
            ...
            it("transfers the ownership of the given token ID to the given address", async function () {
              expect(await this.erc721a.ownerOf(this.tokenId)).to.be.equal(
                this.to
              );
            });
            ...
            it("clears the approval for the token ID", async function () {
              expect(await this.erc721a.getApproved(this.tokenId)).to.be.equal(
                ZERO_ADDRESS
              );
            });
      ...
      context("mint", async function () {
        ...
        describe("safeMint", function () {
          it("successfully mints a single token", async function () {
            ...
            expect(await this.erc721a.ownerOf(this.startTokenId)).to.equal(
              this.receiver.address
            );
          });
        ...
        describe("mint", function () {
          ...
          it("rejects mints to the zero address", async function () {
            await expect(this.erc721a.mint(ZERO_ADDRESS, 1)).to.be.revertedWith(
              "MintToZeroAddress"
            );
          });
          ...
          it("requires quantity to be greater than 0", async function () {
            await expect(
              this.erc721a.mint(this.owner.address, 0)
            ).to.be.revertedWith("MintZeroQuantity");
          });
        });
      });
